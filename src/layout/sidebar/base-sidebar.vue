...
export default defineComponent({
  ...
  setup(props) {
   ...
   const filteredMenus = computed(() => {
    const filterMenusByUserType = (menus: RouteRecordRaw[]): RouteRecordRaw[] => {
      return menus
        .filter(menu => {
          const hideTypes = menu.meta?.hideForUserTypes ?? [];
          return !hideTypes.includes(currentUserType);
        })
        .map(menu => {
          if (menu.children && menu.children.length > 0) {
            return {
              ...menu,
              children: filterMenusByUserType(menu.children)
            };
          }
          return menu;
        });
    ...
    return filterMenusByUserType(state.menus);
  });
    onMounted(() => {
    ...
    return { elm, props, state,filteredMenus  };
  }
});
</script>
...
<template>
  ...
  >
    <sidebar-menus-items :className="state.popClassName" :menus="filteredMenus" :hiddenIndex="state.hiddenIndex"></sidebar-menus-items>
  </el-menu>
</template>
