...
const router = createRouter({
  history: createWebHistory('/app'), //createWebHashHistory() hash模式
  routes: baseRoutes
});
    ...
    if (store.state.routes.length) {
      ...
      if (token) {
        store.initApp().then((res: Array<RouteRecordRaw>) => {
          const baseRoute = toLangRoutes(baseRoutes, initI18n.global.t);
          const mergeRoute = baseRoute.concat(res).concat(errorRoute);
          ...
          store.updateState({
            appIsReady: true,
            routes: mergeRoute,
            routeToMeta: { ...store.state.routeToMeta, ...getBaseRouteToMeta(baseRoute) }
          });
          setTimeout(() => {
        ...
        if (isPop) {
          if (!to.matched.length) {
            ...
            if (to.meta.requiresAuth) {
              next({ path: "/login" }); // ✔ 在 router base 正确时安全
            } else {
              next();
            }
          }
